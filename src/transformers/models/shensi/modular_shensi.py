...
class ShensiAttentionResidual(nn.Module):
    ...
    def forward(
        ...
        gates = F.linear(state, self.gate_proj.weight.float(), self.gate_proj.bias.float())
        decay, erase, write = torch.sigmoid(gates.reshape(*state.shape[:-1], 3, -1)).unbind(-2)
        ...
        updated = forgotten - khat * r + write * (delta if delta is not None else 0.0)
